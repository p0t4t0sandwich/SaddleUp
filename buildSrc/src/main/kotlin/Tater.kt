...
val Project.mergeMixinConfigs: (List<File>)  -> FileTree get() = { mixinConfigs ->
    ...
    val merged = file("./build/tmp/saddleup.mixins.json")
    val mixinConfigJson = mutableMapOf(
        ...
        "required" to false,
        "minVersion" to "0.8",
        "plugin" to "dev.neuralnexus.saddleup.mixin.plugin.SaddleUpMixinPlugin",
        "package" to "dev.neuralnexus.saddleup.mixin",
...
val Project.bundleJars: (List<File>) -> List<FileTree> get() = { jars ->
    val zipped = jars.map { zipTree(it).matching { exclude("saddleup.mixins.*") } }
    val mixinConfigs = mutableListOf<File>()
    jars.forEach { jar ->
        zipTree(jar).files.forEach { file ->
            if (file.name.startsWith("saddleup.mixins.")) {
